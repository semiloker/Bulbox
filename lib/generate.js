...
const sep = () => pick(['', '', '-', '_']); // weighted toward none / hyphen
...
export function randomNickname() {
  ...
  if (style < 45) {
    ...
    else h = p + pick(['', '_', '-']) + pick(NOUN) + (chance(45) ? num() : '');
  } else if (style < 85) {
    // adjective + noun + suffix
    h = pick(ADJ) + sep() + pick(NOUN) + pick(['', '-', '_']) + pick(SUFFIX);
  } else {
    // noun + noun / word + number
    h = pick(NOUN) + sep() + pick(NOUN) + (chance(60) ? num() : '');
  }
  const cleaned = h.replace(/[^a-zA-Z0-9._-]/g, '').slice(0, 20);
